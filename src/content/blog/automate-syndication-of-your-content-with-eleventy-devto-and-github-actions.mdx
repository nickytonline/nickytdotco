...
excerpt: >-
  This started off as a test post for my talk I gave this past week at the
  Eleventy Meetup.      Here’s...
...
tags:
  ...
  - webdev
cover_image: >-
  /assets/images/posts/_dynamic_image_width=1000,height=420,fit=cover,gravity=auto,format=auto_https%3A%2F%2Fdev-to-uploads.s3.amazonaws.com%2Fuploads%2Farticles%2F2x523d97n7fem0pci10s.png
canonical_url: "https://www.nickyt.co/blog/automate-syndication-of-your-content-with-eleventy-devto-and-github-actions"
dev_to_slug: "my-eleventy-meetup-talk-3b2p"
...
DEV uses [liquid tags](https://shopify.github.io/liquid/basics/variations/) for embedding content in blog posts. For those interested, here is the [complete list of supported embeds](https://dev.to/p/editor_guide#liquidtags) via the DEV `<Embed url="url" />` liquid tag.

I'm using [short codes](https://www.11ty.dev/docs/shortcodes/) in Eleventy which are the same syntax as liquid tags. In the past DEV had specific liquid tags for different embeds. For example, to embed a GitHub repository, you'd use the `<GitHubEmbed url="url" />` liquid tag. The liquid tag is still supported, but they now have a generic embed liquid tag, `<Embed url="url" />` which determines what type of embed based on the URL.

In my project, I have shortcodes for specific embeds, e.g. `<GitHubEmbed url="url" />`, `<TwitterEmbed tweetId="url" />`, etc. I have older posts that use the old liquid tags of DEV, but newer posts use the `<Embed url="url" />` liquid tag. On my end I manipulate the markdown to convert e.g. `<Embed url="https://twitter.com/nickytonline/status/1521650477674471424" />` to `<TwitterEmbed tweetId="https://twitter.com/nickytonline/status/1521650477674471424" />`
...
![Boost links for DEV and Hashnode for a blog post of mine](/assets/images/posts/_uploads_articles_6rajggtd3owzd4q8d1ub.png)
