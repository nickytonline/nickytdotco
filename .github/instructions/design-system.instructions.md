...
- Theme UI is a compact fan menu in `ThemeToggle.astro`: one trigger showing the active theme, with Light / Dark / Windows 95 options in a `radiogroup` that opens beside the trigger on `md+` and below on small screens. Light/dark use a rounded pill menu; Windows 95 stays square/chunky. Use a generic four-pane window glyph for Windows 95 — not the Microsoft Windows logo.
