...
- Theme UI is a compact fan menu in `ThemeToggle.astro`: one trigger showing the active theme, with Light / Dark / Windows 95 options in a `radiogroup` that opens beside the trigger on `md+` and below on small screens. Light/dark options are individual full circles attached to the trigger (centered); Windows 95 stays a square/chunky block. Use a generic four-pane window glyph for Windows 95 — not the Microsoft Windows logo.
- In `.windows95`, put readable text on silver surfaces (`[data-site-header]` / `main` / `[data-site-footer]`); teal `--background` is desktop chrome only. Do not apply raised window chrome to in-page `<header>`/`<footer>` blocks (e.g. tag footers) or soft `.tag-pill` links. Navy/black on silver meets AA; do not place `text-brand` or `text-muted-foreground` on teal.
