...
function getSeriesName(collection_id, title) {
  ...
  console.warn(
    `Could not infer series name for collection ${collection_id}, title: "${title}"`
  );
...
async function fetchWithRetry(
  ...
  while (attempt <= maxRetries) {
    ...
    console.warn(
      `Rate limited (429). Retrying in ${waitTime / 1000}s (attempt ${attempt}/${maxRetries})...`
    );
...
async function syncAllPostsWithSeries() {
  ...
  for (const file of mdFiles) {
    ...
    try {
      ...
      if (!match) {
        console.log(
          `${processed}/${mdFiles.length} ⚠️  ${file}: No frontmatter found`
        );
        skipped++;
        continue;
      }
      ...
      if (frontmatter.series && frontmatter.series.collection_id) {
        console.log(
          `${processed}/${mdFiles.length} ⏭️  ${file}: Already has series data`
        );
        skipped++;
        continue;
      }
      ...
      console.log(
        `${processed}/${mdFiles.length} 🔍 ${file}: Fetching from dev.to...`
      );
      const article = await fetchArticleBySlug(parsed.username, parsed.slug);

      if (!article) {
        console.log(
          `${processed}/${mdFiles.length} ❌ ${file}: Failed to fetch`
        );
        errors++;
        continue;
      }
      ...
      if (!article.collection_id) {
        console.log(
          `${processed}/${mdFiles.length} ℹ️  ${file}: Not in a series`
        );
        skipped++;
        continue;
      }
      ...
      if (success) {
        console.log(
          `${processed}/${mdFiles.length} ✅ ${file}: Added to series "${seriesName}" (ID: ${article.collection_id})`
        );
        updated++;
      } else {
        console.log(
          `${processed}/${mdFiles.length} ❌ ${file}: Failed to update`
        );
        errors++;
      }
      ...
      console.error(
        `${processed}/${mdFiles.length} ❌ ${file}: Error - ${error.message}`
      );
      errors++;
    }
  }
