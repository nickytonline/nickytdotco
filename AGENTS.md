...
- Routes live in `src/pages/`, layouts in `src/layouts/`, and UI components in `src/components/`.
- Content lives in `src/content/` and `src/blog/`; live collections are defined in `src/live.config.ts`.
...
- `npm run lint` / `npm run lint:fix`: run ESLint (and fix when desired).
- `npm run format` / `npm run format:check`: run Prettier and ESLint (or check only).
...
- Use Conventional Commits (e.g., `feat:`, `fix:`, `docs:` or `feat(search):`).
- Discuss changes with maintainers before opening a PR.
